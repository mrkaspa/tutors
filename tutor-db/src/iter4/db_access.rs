...
pub async fn get_courses_for_tutor_db(
    ...
    if courses.len() == 0 {
        Err(AppError::NotFound("courses not found".into()))
    } else {
        Ok(courses)
    }
}
...
pub async fn get_course_details_db(
    ...
    if let Ok(course_row) = course_row {
        ...
        Err(AppError::NotFound("course not found".into()))
    }
}

pub async fn post_new_course_db(pool: &PgPool, new_course: Course) -> Result<Course, AppError> {
    let course_row = sqlx::query!("insert into ezy_course_c4 (course_id,tutor_id, course_name) values ($1,$2,$3) returning tutor_id, course_id,course_name, posted_time", new_course.course_id,
    ...
    if let Ok(course_row) = course_row {
        ...
        Err(AppError::NotFound("course could not be created".into()))
    }
}
